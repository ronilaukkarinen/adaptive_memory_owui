...
Analyze the following related memories and provide a concise summary.""",
        ...
        # Timeout settings
        memory_identification_timeout: int = Field(
            default=120,
            description="Timeout in seconds for memory identification process (prevents infinite hangs)"
        )
...
Your output must be valid JSON only. No additional text.""",
        ...
        @field_validator(
            ...
            'memory_identification_timeout', # Added
        )
    ...
    def __init__(self):
        ...
        self.error_timestamps = {
            ...
            "llm_timeout_errors": deque(),  # Added timeout error tracking
    ...
    async def _process_user_memories(
        ...
        try:
            ...
            try:
                # Add timeout protection for memory identification
                identification_timeout = self.valves.memory_identification_timeout
                memories = await asyncio.wait_for(
                    self.identify_memories(
                        user_message,
                ...
                logger.error(f"Memory identification timed out after {identification_timeout} seconds")
                self.error_counters["llm_call_errors"] += 1
                self.error_timestamps["llm_timeout_errors"].append(time.time())  # Track timestamp for guard
                self._error_message = "memory_identification_timeout"
                if show_status:
                    await self._safe_emit(
                        event_emitter,
                        {
                            "type": "status",
                            "data": {
                                "description": f"⚠️ Memory extraction timed out after {identification_timeout} seconds - skipping memory save.",
                                "done": True,
                            },
                        },
                    )
...
Current datetime: {current_datetime.strftime('%A, %B %d, %Y %H:%M:%S')} ({current_datetime.tzinfo})"""
    ...
    async def query_llm_with_retry(self, system_prompt: str, user_prompt: str) -> str:
        ...
        # Implement overall timeout for the entire function
        overall_timeout = min(300, 60 * (max_retries + 1))  # 5 minutes max, or 60s per retry

        try:
            return await asyncio.wait_for(
                self._query_llm_with_retry_internal(
                    provider_type, model, api_url, headers, system_prompt_with_date,
                    user_prompt, max_retries, retry_delay, session
                ),
                timeout=overall_timeout
            )
        except asyncio.TimeoutError:
            logger.error(f"LLM query timed out after {overall_timeout} seconds")
            return f"Error: LLM query timed out after {overall_timeout} seconds"
        except Exception as e:
            logger.error(f"Unexpected error in LLM query: {e}")
            return f"Error: Unexpected LLM error: {str(e)}"

    async def _query_llm_with_retry_internal(
        self, provider_type: str, model: str, api_url: str, headers: dict,
        system_prompt_with_date: str, user_prompt: str, max_retries: int,
        retry_delay: float, session
    ) -> str:
        """Internal implementation of LLM query with retry logic"""
        ...
        for attempt in range(1, max_retries + 2):  # +2 because we start at 1 and want max_retries+1 attempts
            ...
            try:
                ...
                # Make the API call with shorter timeout per request
                request_timeout = 45  # Shorter timeout per request
                ...
                logger.warning(f"Attempt {attempt} failed: LLM API request timed out after {request_timeout}s")
                if attempt <= max_retries:
                    ...
                    return f"Error: LLM API request timed out after {max_retries + 1} attempts (each {request_timeout}s)."
                ...
                if attempt <= max_retries:
                    ...
                    return f"Error: LLM_CONNECTION_FAILED after {max_retries + 1} attempts: {str(e)}"
            except Exception as e:
                logger.error(
                ...
                if attempt <= max_retries:
                    ...
                    return f"Error: UNEXPECTED_LLM_ERROR after {max_retries + 1} attempts: {str(e)}"

        return f"Error: LLM query failed after {max_retries + 1} attempts."
